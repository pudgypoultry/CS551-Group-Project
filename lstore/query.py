...
class Query:
    ...
    def select(self, search_key, search_key_index, projected_columns_index):
        # FIXME: reinstate try/except when debugging is complete
        # try:
            records = []
            RIDs = self.table.index.locate(search_key_index, search_key)
            if RIDs == None:
                print(f"RIDs is null with search_key_index={search_key_index} and search_key={search_key}")
            for rid in RIDs:
    ...
    def select_version(self, search_key, search_key_index, projected_columns_index, relative_version):
        ...
        for rid in RIDs:
            ...
            if len(self.table.recordDirectory[rid]) < abs(relative_version):
                ...
                record = self.table.fetch(rid, relative_version) 
    ...
    def sum(self, start_range, end_range, aggregate_column_index):
        ...
        for t in range(start_range, end_range + 1):
            ...
            try:
                ...
                print(f"sum {sum} + value {val_to_add}")
                sum += val_to_add
                record_exists = True
            except:
                continue
