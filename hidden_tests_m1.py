    ...
    for key in records:
        ...
        for j, column in enumerate(record.columns):
            if column != updated_records[key][j]:
                error = True
        if error:
            print('update error on', records[key], 'and', updated_columns, ':', record.columns, ', correct:',
                  records[key])
        else:
            pass
    score = score + 15
    ...
    for c in range(0, grades_table.num_columns):
        for i in range(0, number_of_aggregates):
            ...
            if updated_column_sum != updated_result:
                raise Exception('sum error on column', c, '[', keys[r[0]], ',', keys[r[1]], ']: ', updated_result,
                                ', correct: ', updated_column_sum)
            else:
                pass
    score = score + 15
